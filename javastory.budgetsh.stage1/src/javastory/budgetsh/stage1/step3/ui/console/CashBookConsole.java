...
public class CashBookConsole {
	...
	public CashBook register() {
		...
		while(true) {
			...
			if(cashBookMapStore.exist(bankAccount)) {
				narrator.sayln("already exist -->" + bankAccount);
				continue;
			}
			...
			socialian = new Socialian(leaderSocialId, leaderFirstName,
					leaderFamilyName,leaderEmail);
			...
			cashBookMapStore.regist(cashbook);
	...
	public CashBook find() {
		...
		if (cashBookMapStore.exist(account)) {
			cashbook = cashBookMapStore.retrieve(account);
			narrator.sayln("\n > Found Cashbook" + cashbook.getClub().getName());
			return cashbook;
		}else {
	...
	public CashBook modify() {
		...
		if(foundCashbook!=null) {
			...
			cashBookMapStore.update(foundCashbook);
	...
	public void remove() {
		...
		if(cashBookMapStore.exist(bankAccount)) {
			foundCashbook = cashBookMapStore.retrieve(bankAccount);
			cashBookMapStore.remove(foundCashbook);
